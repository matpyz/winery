...
class GuardianView extends JFrame implements ActionListener {
	...
	private void createGUI(int width, int height) {
		...
		txt_login.setMaximumSize(new Dimension(Integer.MAX_VALUE, txt_login.getPreferredSize().height));
		...
		txt_password.setMaximumSize(new Dimension(Integer.MAX_VALUE, txt_password.getPreferredSize().height));
	...
	/*
	 * public static void main(String[] args) { Guardian model = new Guardian();
